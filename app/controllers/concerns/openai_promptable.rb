module OpenaiPromptable
  ...
  def bankrupcty_step_1(public_record, bureau)
    <<-PROMPT
      You are asked to generate the a letter to the bankruptcy court to ask the following information
  ...
  def bankrupcty_step_2(public_record, bureau)
    <<-PROMPT
    You are asked to generate the a letter to Lexis Nexus dispute center to remove the following information on my report
  ...
  def format_public_records_by_bureau(records, bureau)
    ...
    public_records.first(limit).map do |account|
      details = account[:bureau_details].find { |detail| detail[:bureau].downcase == bureau.downcase }
      <<-DETAILS
        Public Information
