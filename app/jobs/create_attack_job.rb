class CreateAttackJob < ApplicationJob
  ...
  def perform(user, round)
    ...
    personal_information_details = user.personal_informations
    ...
    generate_pdfs(letter, user, inquiry_details, account_details, personal_information_details)
    ...
    CreditReportMailer.reimport(user).deliver_later(wait_until: 32.days.from_now)
  end

  private

  def generate_pdfs(letter, user, inquiry_details, account_details, personal_information_details)
    generate_pdf(letter, 'experian_document', :experian_pdf, user, inquiry_details, account_details, personal_information_details) if letter.experian_document.present?
    generate_pdf(letter, 'transunion_document', :transunion_pdf, user, inquiry_details, account_details, personal_information_details) if letter.transunion_document.present?
    generate_pdf(letter, 'equifax_document', :equifax_pdf, user, inquiry_details, account_details, personal_information_details) if letter.equifax_document.present?
  ...
  def generate_pdf(letter, document_field, pdf_attachment, user, inquiry_details, account_details, personal_information_details)
      ...
      if letter.name.in?(["Bankruptcy Step 1", "Bankruptcy Step 2"])
        ...
        if personal_information_details.any?
          ...
          fields.each do |field|
            ...
            transunion_info = personal_information_details.find { |pi| pi.bureau.downcase == 'transunion' }&.send(field[:name]) || ""
            equifax_info = personal_information_details.find { |pi| pi.bureau.downcase == 'equifax' }&.send(field[:name]) || ""
            experian_info = personal_information_details.find { |pi| pi.bureau.downcase == 'experian' }&.send(field[:name]) || ""
        ...
        if inquiry_details.any?
          ...
          inquiry_details.each_with_index do |inquiry, index|
            ...
            pdf.move_down 5
            pdf.text "<color rgb='ff0000'>*HRCF12</color> #{inquiry[:name]}  |   <color rgb='ff0000'>*HRCF8</color> #{inquiry[:date_of_inquiry]}</color>  | <color rgb='ff0000'>^^HRCF5-</color> #{inquiry[:bureau]}", inline_format: true, size: 10
            pdf.move_down 4
            pdf.text "#{inquiry[:name]} hypothetic on #{inquiry[:date_of_inquiry]} necessary adequate and urgent achieved lawful Permissible Purpose, and or is not testimonial obedient as it is currently reported. Please REMOVE from reporting immediately!", size: 10
            pdf.text "------------------------------------------------------------------------------------------------------"
            pdf.move_down 20
          end
        end
        ...
        if account_details.any?
          ...
          vertical_headers = [
            ...
            "Date of Last Payment <color rgb='ff0000'>* #{Letter::METRO_2_COMPLIANCE_CODES.key('date_of_last_payment')}</color>",
            "Last Reported <color rgb='ff0000'>* #{Letter::METRO_2_COMPLIANCE_CODES.key('last_reported')}</color>"
          ]
